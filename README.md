...
### Device Motion (Android, iOS, Browser)
...
### Device Orientation (Android, iOS, Browser)
...
### Device eMagnetometer (Android, iOS)

자력계 는 특정 위치에서 자기장 의 방향, 강도 또는 상대적인 변화를 측정하는 장치입니다. 자성 물질 ( 페로 마그네 와 같은)의 자화 측정은 예입니다. 나침반 은 주변 자기장의 방향 (이 경우 지구 자기장)을 측정하는 장치 중 하나입니다.
...
### Document Viewer (Android, iOS)

이 플러그인은 앱 자산 폴더 (/ www / *) 또는 cordova 파일 플러그인을 통해 사용 가능한 다른 파일 시스템 디렉토리에 저장된 PDF 파일을 볼 수있는 슬림 API를 제공합니다.

### Downloader (Android)

이 플러그인은 Android DownloadManager를 사용한 파일 다운로드를 지원하도록 설계되었습니다.

### Email Composer (Android, iOS, Browser)

플러그인은 편집 및 이메일 메시지 전송을 관리하는 표준 인터페이스에 대한 액세스를 제공합니다. 이보기 컨트롤러를 사용하여 애플리케이션 내부에 표준 이메일보기를 표시하고 제목, 이메일 수신자, 본문 및 첨부 파일과 같은 초기 값으로 해당보기의 필드를 채울 수 있습니다. 사용자는 지정한 초기 내용을 편집하고 이메일을 보내거나 작업을 취소하도록 선택할 수 있습니다.

이 인터페이스를 사용한다고해서 해당 이메일 메시지가 즉시 전달되는 것은 아닙니다. 사용자는 메시지 작성을 취소 할 수 있으며 사용자가 메시지를 보내도록 선택하면 메시지는 메일 응용 프로그램 발신함에 만 대기됩니다. 이를 통해 비행기 모드와 같이 사용자가 네트워크에 액세스 할 수없는 상황에서도 이메일을 생성 할 수 있습니다. 이 인터페이스는 전자 메일이 실제로 전송되었는지 확인할 수있는 방법을 제공하지 않습니다.

### Emm App Config (Android, iOS)
